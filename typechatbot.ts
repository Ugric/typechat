...
client.on('ready', () => {
    console.log(`Logged into discord as ${client.user.tag}!`);client.user.setActivity('TypeChat💬', { type: 'PLAYING' });
});
    ...
    if(!message.guild) {
        ...
        if (["!unlink", "!link"].includes(message.content)) {
            if (message.content === "!unlink") {
                ...
                message.reply({ embeds: [new discord.MessageEmbed().setColor("#5656ff").setTitle("Unlinked 🔓")] })}
                else {
                    message.reply({ embeds: [new discord.MessageEmbed().setColor("#5656ff").setTitle("Not Linked")] })
                }
            } else if (message.content === "!link") {
                if (!link) {
                    message.reply({ embeds: [linkaccount(message.author)] })
                } else {
                    message.reply({ embeds: [new discord.MessageEmbed().setColor("#5656ff").setTitle("Already Linked")] })
                }
            }
        } else if (message.content === "!help") {
            message.reply({ embeds: [new discord.MessageEmbed().setColor("#5656ff").setTitle("DM Commands").addFields(DMCommandsList)] })
        }
    } else {
        if (message.content === "!help") {
            message.reply({ embeds: [new discord.MessageEmbed().setColor("#5656ff").setTitle("Server Commands").setDescription("NO SERVER COMMANDS HAVE BEEN CREATED YET!")] })
        }
    }
})
