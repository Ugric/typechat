...
function MessageMaker({
  ...
  const cache = useRef(
    new CellMeasurerCache({ fixedWidth: true, defaultHeight: 15 })
  );
  ...
  return (
    <div className="chats">
      {messages.length > 0 ? (
        <>
          <List
            style={{ padding: `110px 0 4rem 0` }}
            width={size.width < 900 ? size.width : 900}
            height={size.height - (navbarsize.height + (isElectron() ? 30 : 0))}
