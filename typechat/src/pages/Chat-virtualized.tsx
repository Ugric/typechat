...
function MessageMaker({
  ...
  const { user, navbarsize } = useData();
  const cache = useRef(new CellMeasurerCache({ fixedWidth: true, defaultHeight: 15 }))
  const { users } = useContext(usersContext);
  const size = useWindowSize()
  /*
        output.push(
          <CellMeasurer cache={cache.current}
            key={ messages[i].time }><p
            style={ {
              margin: "0",
              color: `lightgray`,
              fontSize: "10px",
  ...
  return (
    <div className="chats" >
      { messages.length > 0 ? (
        <><List style={ { padding: `110px 0 4rem 0` } } width={ size.width < 900 ? size.width : 900 } height={ size.height - (navbarsize.height + (isElectron() ? 30 : 0)) } deferredMeasurementCache={ cache.current } rowHeight={ cache.current.rowHeight } rowCount={ messages.length }
          rowRenderer={ ({ index, style, parent }) => {
            return <CellMeasurer cache={ cache.current } key={ messages[index].ID ? messages[index].ID : messages[index].tempid } parent={ parent } columnIndex={ 0 } rowIndex={ index }>
              <div style={ style } >
                <Message
                  messages={ messages }
                  deleteFromID={ deleteFromID }
...
function ChatPage() {
  ...
  const history = useHistory();
  const { id: chattingto } = useParams<{ id: string }>();
  const { notifications, user, NotificationAPI, navbarsize } = useData();
        ...
        <div>
          <div
            style={ {
              ...
              width: "100%",
              height: "90px",
              top: (navbarsize.height + (isElectron() ? 30 : 0)) + "px",
              padding: "10px",
              zIndex: 10,
              background:
